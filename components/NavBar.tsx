...
export default function NavBar() {
  ...
  // Sync input with URL param when on /galeria
  useEffect(() => {
    if (isGaleria) {
  ...
  // Hide search bar on non-gallery pages (e.g. /upload, /perfil)
    ...
    <nav className="sticky top-0 z-40 bg-white border-b border-gray-100">
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        <div className="flex items-center gap-3 h-16">
          ...
          {/* Search — central, only on gallery/landing */}
          {showSearch && (
            <div className="relative flex-1 max-w-2xl mx-auto hidden sm:block">
              ...
              <input
                ...
                onKeyDown={e => {
                  if (e.key === 'Enter') {
                    if (isLanding) navigate(inputValue)
        ...
        {/* Mobile search row — only on gallery */}
        {showSearch && (
